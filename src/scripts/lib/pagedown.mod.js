...
(function () {
	...
	Markdown.Converter = function (opt) {
		opt = opt || {} // add by Wilson
		...
		function _FormParagraphs(text, doNotUnhash) {
			...
			for (var i = 0; i < end; i++) {
				...
				if (markerRe.test(str)) {
					...
					if (opt.soft_break) str = str.replace(/\n/g,"<br />");  // ** GFM **
					str = str.replace(/^([ \t]*)/g, "<p>");
					str += "</p>"
					grafsOut.push(str);
				}
		...
		//
			// disabled by wilson for it useless on client side
	...
	// mod by Wilson to support email
	...
	gfm.hooks.chain("preConversion", function (text) {
		// ** GFM **  Auto-link URLs and emails: already has
	...
	Markdown.md = function (code) {
		return pagedown.makeHtml(code);
	}
	Markdown.gfm = function (code) {
		return gfm.makeHtml(code);
	}
})();
