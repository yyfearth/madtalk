...
(function() {
  var Login, add_log, channel, do_login, gfm, login, user, xss_safe, _log, _toolbar, _users;
  ...
  */
  ...
  add_log = function(recs) {
    ...
    recs = recs.map(function(rec) {
      return "<li><label>" + (xss_safe.str(rec.user.nick)) + "<br/>" + (new Date(rec.ts).toLocaleTimeString()) + "</label><div>" + (gfm.makeHtml(xss_safe.str(rec.data))) + "</div></li>";
    });
